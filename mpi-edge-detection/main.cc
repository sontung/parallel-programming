...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < files.size(); i++) {
        sprintf(filename, "%s%s", argv[1], files[i].c_str());
        sprintf(outputname, "%s%s", "/home/sontung/Downloads/blog/media3/", files[i].c_str());
        std::string name(filename);
        std::size_t check_filename = name.find(".png");
        if (check_filename == std::string::npos) continue;
        if (world_rank == 0) printf("Processing %s%s\n", argv[1], files[i].c_str());
