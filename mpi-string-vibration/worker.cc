...
float * simulate(const float alpha, const long n_segments, const int n_steps, float *d_buf1, float *d_buf2, const int rank, const int world_size, const long segments_per_process) {
  ...
  for(int t = 0; t < n_steps; t++) {
    ...
    if (rank > 0) MPI_Send(&d_t[start_segment], 1, MPI_FLOAT, rank-1, 0, MPI_COMM_WORLD);    
    if (rank < world_size-1) MPI_Send(&d_t[last_segment-1], 1, MPI_FLOAT, rank+1, 0, MPI_COMM_WORLD);

    if (rank > 0) MPI_Recv(&d_t[start_segment-1], 1, MPI_FLOAT, rank-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    if (rank < world_size-1) MPI_Recv(&d_t[last_segment], 1, MPI_FLOAT, rank+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Barrier(MPI_COMM_WORLD);

    //MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, &d_t[start_segment-1], 1, MPI_FLOAT, MPI_COMM_WORLD);
    //
    //MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, &d_t[last_segment], 1, MPI_FLOAT, MPI_COMM_WORLD);
  }
  MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, &d_t[1], segments_per_process, MPI_FLOAT, MPI_COMM_WORLD);
  return d_t;
}
